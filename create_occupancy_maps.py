...
def get_feature_map(task, voxel_size=0.05, max_images=500, room_height=2.75):
    ...
    feature_map = torch.zeros(size0, size1, size2, 1).cuda()
    ...
    for config in valid_positions[:max_images]:
        ...
        with torch.no_grad():
            ...
            image_features = torch.tensor(obs["depth"]).cuda()
...
if __name__ == "__main__":
    ...
    os.makedirs("occupancy_maps", exist_ok=True)
...
{prefix} {feature_map.shape[0]} / {np.prod(hits_per_voxel.shape)} voxels are occupied")
            ...
            np.save(os.path.join(
                "occupancy_maps", f"{prefix}-coords.npy"), coords)
            np.save(os.path.join(
                "occupancy_maps", f"{prefix}-feature_map.npy"), feature_map)

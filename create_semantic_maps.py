...
def get_semantic_map(task, voxel_size=0.05, max_images=500, feature_size=len(OBJECT_NAMES), room_height=2.75):
    ...
    for config in valid_positions[:max_images]:
        ...
        with torch.no_grad():
            ...
            semantic_image_colors = task.env.last_event.semantic_segmentation_frame
            unique_colors = np.unique(semantic_image_colors.reshape(224 * 224, 3), axis=0)

            semantic_image_classes = np.zeros([224, 224, len(OBJECT_NAMES)])

            for unique_color in unique_colors:

                if tuple(unique_color.tolist()) not in task.env.last_event.color_to_object_id:
                    continue

                object_id = task.env.last_event.color_to_object_id[tuple(unique_color.tolist())]
                ...
                semantic_image_classes[ind_y, ind_x, ind_c] = 1.0

            image_features = torch.FloatTensor(semantic_image_classes).cuda()

            focal_length = (224 / 2.0 / np.tan(np.radians(90.0) / 2.0))

            rays = project_camera_rays(224, 224, focal_length, focal_length).cuda()
            rays = transform_rays(rays, eye_vector, up_vector)

            depth = torch.tensor(obs["depth"]).cuda()
...
if __name__ == "__main__":
    ...
    for stage in ["train", "val", "test"]:
        ...
        for task_id in range(num_tasks):
            ...
            coords, feature_map, hits_per_voxel = get_semantic_map(task)

            coords = coords[..., 0, :2]
            feature_map = feature_map.sum(axis=2)
            ...
            print(f"[{stage}: {task_id}/{num_tasks}] {prefix} {feature_map.shape[0]} voxels")

            np.save(os.path.join(
                "semantic_maps", f"{prefix}-coords.npy"), coords)
            np.save(os.path.join(
                "semantic_maps", f"{prefix}-feature_map.npy"), feature_map)

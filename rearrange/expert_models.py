...
class ExpertConvRNN(ActorCriticModel[CategoricalDistr]):
    ...
    def __init__(
        ...
        """Initialize a `ExpertConvRNN` object.
    ...
    def forward(  # type:ignore
        ...
        big_x = torch.cat([x, observations["map"]["goal_feature"], self.pos_encoding(
            observations["map"]["goal_position"])], dim=-1)

        ac_output = ActorCriticOutput(
            distributions=self.actor(big_x), values=self.critic(x), extras={}
        )

        return ac_output, memory.set_tensor("rnn", rnn_hidden_states)


class PretrainedExpertConvRNN(ExpertConvRNN):
    def __init__(
        self,
        action_space: gym.spaces.Discrete,
        observation_space: gym.spaces.Dict,
        ...
        Nearly identical to `PretrainedExpertConvRNN` but
